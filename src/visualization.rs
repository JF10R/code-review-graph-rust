...
//!
//! Generates a self-contained HTML file with embedded graph data and a
...
use crate::types::node_to_dict;

/// Export all graph nodes and edges as a JSON-serializable value.
///
/// Returns `{"nodes": [...], "edges": [...], "stats": {...}}`.
pub fn export_graph_data(store: &GraphStore) -> Result<Value> {
    ...
    for file_path in store.get_all_files()? {
        for gnode in store.get_nodes_by_file(&file_path)? {
            if seen_qn.contains(&gnode.qualified_name) {
                continue;
            }
            ...
            if let Some(obj) = d.as_object_mut() {
                obj.insert("params".to_string(), Value::Null);
                obj.insert("return_type".to_string(), Value::Null);
            }
            nodes.push(d);
        }
    }

    // Collect edges: iterate all source nodes and gather their outgoing edges.
    ...
    for qn in &seen_qn {
        for edge in store.get_edges_by_source(qn)? {
            let key = format!("{}->{}:{}", edge.source_qualified, edge.target_qualified, edge.kind.as_str());
            if seen_edges.contains(&key) {
                continue;
            }
            seen_edges.insert(key);

            // Build edge JSON with "source"/"target" keys matching D3 template
            let edge_val = json!({
    ...
    // GraphStats derives Serialize — no manual re-serialization needed.
...
pub fn generate_html(store: &GraphStore, output_path: &Path) -> Result<()> {
    ...
    let html = HTML_TEMPLATE.replace("__GRAPH_DATA__", &data_json);
...
function showTooltip(ev, d) {
  ...
  if (d.line_start != null) h += `<div class="tt-row"><span class="tt-label">Lines: </span>${d.line_start} &ndash; ${d.line_end || d.line_start}</div>`;

...
fn hybrid_query_returns_ok_with_method_field() {
    ...
    let result = hybrid_query("add", 5, Some(&root_str), false, None, None, None).unwrap();
...
fn hybrid_query_empty_query_returns_empty() {
    ...
    let result = hybrid_query("", 10, Some(&root_str), false, None, None, None).unwrap();
    assert_eq!(result["status"], "ok");
...
fn hybrid_query_results_include_rrf_score() {
    ...
    let result = hybrid_query("subtract", 5, Some(&root_str), false, None, None, None).unwrap();
    assert_eq!(result["status"], "ok");
